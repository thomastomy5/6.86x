...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    # Your code here
    loss = np.dot(feature_vector, theta.T) + theta_0
    loss *= label
    if loss > 1:
        return 0
    else:
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    # Your code here
    distance = np.matmul(feature_matrix, theta.T) + theta_0
    distance *= labels
    loss = np.where(distance > 1, 0, 1 - distance)
    return np.mean(loss)
...
def perceptron_single_step_update(
    ...
    """
    # Your code here
    if label * (np.dot(feature_vector, current_theta.T) + current_theta_0) <= 0.00001:
...
def perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            x = feature_matrix[i]
            y = labels[i]
            if y*(np.dot(x,theta.T)+theta_0) <= 0:
                theta+= x*y
                theta_0+= y
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            ...
            thetas+= theta
            thetas_0+= theta_0
    return (thetas/(n*T),thetas_0/(n*T))
...
def pegasos_single_step_update(
    ...
    x = feature_vector
    y = label
    if y * (np.dot(x, current_theta.T) + current_theta_0) <= 1:
        current_theta = (1-eta*L)*current_theta + eta*y*x
        current_theta_0 += eta*y
    else:
